            ...
            >
              ...
              <div className="flex items-end justify-center" style={{ gap: '2px' }}>
                {handCards.map((card, index) => {
                  ...
                  const isCardLandscape = card.width > 0 && card.height > 0 && card.width > card.height;
                  const baseW = isMobileLandscape ? 45 : (isMobile ? 60 : 80);
                  const baseH = isMobileLandscape ? 63 : (isMobile ? 84 : 112);
                  const cardWidth = isCardLandscape ? baseH : baseW;
                  const cardHeight = isCardLandscape ? baseW : baseH;
      ...
      {hoveredHandCard && !draggingFromHand && (() => {
        ...
        return (
          <div className="fixed z-50 pointer-events-none" data-testid="hand-card-preview" style={{ left: '50%', top: '50%', transform: 'translate(-50%, -70%)' }}>
            <div className="rounded-xl overflow-hidden border-2 border-yellow-400 shadow-2xl shadow-black/50" style={{ width: getCardDims(card).w * 2, height: getCardDims(card).h * 2, backgroundColor: '#fff' }}>
              {card.image_path ? (
                <img src={card.image_path} alt={card.name} className="w-full h-full object-contain" />
              ) : (
                <div className="w-full h-full flex flex-col items-center justify-center bg-gray-100">
      ...
      {draggingFromHand && (() => {
        ...
        return (
          <div
            ...
            style={{
              left: handDragPosition.x - getCardDims(card).w / 2,
              top: handDragPosition.y - getCardDims(card).h / 2,
            }}
          >
            <div
